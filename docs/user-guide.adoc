...
The AsciiDoc XML Converter is a tool for converting AsciiDoc documents into XML, HTML, and XHTML formats. It provides multiple interfaces for different use cases:
...
3. Click **Convert** (or wait for auto-convert if enabled)
4. View the result in the output panel

==== Validating Syntax

Click the **Validate** button to check your AsciiDoc syntax without converting. You'll see a success message for valid documents or an error message describing any syntax issues.

==== Viewing Output

* **Rendered View**: See the final HTML output as it would appear in a browser
* **Source View**: Click the "Source" tab to view the raw HTML/XML code with syntax highlighting
...
* Click file icons (📄) to load and preview the file
